...
class IggyError : public std::runtime_error {
public:
  IggyError(std::string operation, ErrorCategory category, std::string message);
...
::rust::Vec<std::uint8_t> to_rust_bytes(const std::vector<std::uint8_t> &value);
std::vector<std::uint8_t> from_rust_bytes(const ::rust::Vec<std::uint8_t> &value);
...
public:
  ...
  IggyClient &login_user(const std::string &username,
  ...
  IggyClient &create_topic(const Identifier &stream, const std::string &name,
                           std::uint32_t partitions_count,
                           const std::string &compression_algorithm = std::string(),
                           std::uint8_t replication_factor = 0) &;
  IggyClient &&create_topic(const Identifier &stream, const std::string &name,
                            std::uint32_t partitions_count,
  ...
  IggyClient &send_messages(const Identifier &stream, const Identifier &topic,
  ...
  std::vector<IggyMessage> poll_messages(const Identifier &stream,
...
private:
  explicit IggyClient(::rust::Box<::iggy::ffi::IggyClient> inner);
